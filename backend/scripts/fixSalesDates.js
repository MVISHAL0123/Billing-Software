...
async function fixSalesDates() {
  try {
    console.log('🔄 Fixing date formats in sales bill records...');
    console.log('');

    // Get all sales/bills
    const salesSnapshot = await db.collection('bills').get();
    
    if (salesSnapshot.empty) {
      console.log('❌ No sales bills found in database');
      return;
    }

    console.log(`📦 Found ${salesSnapshot.size} sales bill records`);
    console.log('');
    ...
    for (const doc of salesSnapshot.docs) {
      ...
      const currentDate = salesData.date;
      
      console.log(`🔍 Checking sales bill: ${salesData.billNo}`);
    ...
    if (updateCount > 0) {
      ...
      console.log('🎉 SALES BILL DATES FIXED!');
      console.log('═'.repeat(60));
      console.log(`📊 Summary:`);
      console.log(`   • Sales bill records processed: ${salesSnapshot.size}`);
      console.log(`   • Records updated: ${updateCount}`);
      console.log(`   • Records already correct: ${salesSnapshot.size - updateCount}`);
      console.log('');
      console.log('📋 All sales bill dates are now properly formatted!');
      console.log('🔄 Refresh sales display to see corrected dates');
      console.log('═'.repeat(60));
    } else {
      console.log('✅ All sales bill dates are already in correct format!');
    }

  } catch (error) {
    console.error('❌ Error fixing sales bill dates:', error);
  }
